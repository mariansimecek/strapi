...
const EditViewPage = () => {
   ...
   */
  const isCreatingDocument = id === 'create' && !isSingleType;

  const isLoadingActionsRBAC = useDocumentActionsRBAC('EditViewPage', (state) => state.isLoading);
  const { document, isLoading: isLoadingDocument, validate, schema, components } = useDoc();
  ...
  return (
    <Main paddingLeft={10} paddingRight={10}>
      ...
      <TabGroup variant="simple" label="Document version">
        <StatusTabs>
          <StatusTab>DRAFT</StatusTab>
          <StatusTab>PUBLISHED</StatusTab>
        </StatusTabs>
        <TabPanels>
          <TabPanel>
            <Form
